...
def _lowest_group_confidence(logits: torch.Tensor,
                             labels: torch.Tensor,
                             ignore_index: int = -100,
                             group_size: int | None = None) -> torch.Tensor:
    """
    计算每条样本的"最低组置信度"(Lowest Group Confidence)：
    - 对正确 token 的 log prob 做滑窗平均 (窗口大小=group_size)
    - 取该序列窗口均值的最小值 min_avg_logp
    - 返回 p_conf = exp(min_avg_logp) ∈ [0,1]
    """
    import os
    if group_size is None:
        group_size = int(os.getenv("BFT_GROUP_SIZE", "256"))

    # print("BFT_GROUP_SIZE: ",group_size)
    # import sys
    # sys.exit()
    ...
    with torch.no_grad():
        ...
        for b in range(B):
            v = per_tok_logp[b][valid_mask[b]]                           # 仅有效 token
            if v.numel() == 0:
            ...
            if v.numel() < group_size:
                ...
                y = F.conv1d(x, w, stride=1)                             # [1,1,L-g+1]
                min_avg_logp = y.min().squeeze()

            p_conf = torch.exp(min_avg_logp).clamp(0.0, 1.0)             # [0,1]
            p_conf_list.append(p_conf)

        p_conf = torch.stack(p_conf_list)                                # [B]
        return p_conf


def bft_loss_func(outputs, labels, num_items_in_batch=None):
    """
    BFT Loss: 基于样本置信度的自适应DFT加权
    - 对每个样本计算最低组置信度 p_conf ∈ [0,1]
    - loss = mean((1 - p_conf_i) * dft_loss_i)
    """
    import os
    logits = outputs.get("logits")
    if logits is None:
    ...
    p_conf = (p_conf ** gamma).detach()  # [B], 已经在 [0,1] 范围内
    
    # 计算 per-sample 的 DFT loss
    B, T, V = logits.shape
